...
- [x] **승패 판정** (2026-08-24) — 3단계 추정(screp 잔류 휴리스틱 / 이탈 기록 없는 종료=선이탈 패 /
  컴퓨터전 미상), 리포트·전적 표시 + 승률 추세. 이긴 판(Pole Star)으로 검증 ✅
- [x] **채팅 로그** — 리포트·타임라인 표시 (ㅈㅈ 타이밍이 승패 판정과 정합 확인) ✅
- [x] **인터랙티브 타임라인** (.analysis.html, 판마다 자동) — 반복 피드백 4회 수렴:
  상단 고정 오버뷰(경기 전체 한 화면·클릭 점프·현재 구간 창·분당 활동량 음영) +
  고정 높이 상세 창(분당 1행·3구역 나|메시지|상대·빈 분 접기·보조 건물 토글) +
  중앙 메시지 전부 표시(같은 문구 ×n 합침·폭 400px·최대 2줄) + 클릭 ±20초 맥락 ✅
- [x] 과거 대인전 20판 오프라인 분석 — simonZerg 계정 식별, 전적 21판 시드
  (실측 베이스라인: 유효율 나 89% vs 상대 60~75% — "판단력 그대로" 실증) ✅
...
## 기능 고도화 배치 (2026-08-24 — "고도화 먼저" 사용자 지시)

- [x] **앞마당·삼룡이 존 라벨** — MapProfile.expansions 소비, 본진 최근접 1·2번째 확장
  (본진 자원 군집·원거리 제외). 카탈로그 15라벨 × 3문형 ✅
- [x] **지난 판 브리핑** — 분석 완료 시 상태 창에 "지난 판: 승패 · 팁 응답 · 오탐 의심" 한 줄 ✅
- [x] **닷지 필터(C10)** — 1분 미만 판은 전적 추세·승률에서 제외(표에는 표시) ✅
- [x] **타이밍 비교표(C3)** — 리포트에 "나 vs 상대: 첫 확장 + 공통 건물·테크 Δ초" (1:1 한정) ✅
- [x] **랜덤 종족 인게임 확정 (사용자 통찰)** — 인구 카운터 옆 종족 아이콘 템플릿 매칭
  (P·T 실측 16×16, SAD 임계 35 — P↔T 차 39). 저그는 소거법 잠정(첫 저그 실기에서 템플릿 실측).
  supply.block 문구가 랜덤에서도 종족별로 살아남 ✅
- [ ] 저그 아이콘 템플릿 실측 (저그 플레이 1판 — 소거법 검증 겸)
- [ ] 랜덤 색 확인: 동맹창 스와치 경로는 기존 구현 ✅ (5단계 AllianceReader)
- [x] **시프트+탭 대응 (사용자 실플레이)** — ① 내 색 인게임 관측(시작 3초, 본진 주변
  최다 채도색 → ColorTable mine 병행 기준: 개별 색 모드에서도 피격·정찰 생존)
  ② FlashDetector 팔레트 전환 가드(토글 ≥ 가시 셀 80% && 40셀↑ = 전역 전환 → 이력 리셋,
  연타 재발 오발 차단 — 총량 기준은 깜빡임 실측과 충돌해 기각) ✅
- [x] **실전 1차 튜닝 (2026-08-24 밤 7판)** — 실사용 확정 결함 2건 수정:
  ① 게임 이탈 = 종료 인식 (점수 화면 빠른 통과로 7판 전부 확정 실패 → inGame 60s+ 후
  로비·idle 이탈 시 분석 트리거) ② 미지 색 추론 §6.4-5 구현 (빨무 5인 개별 색에서
  적 3명 미검출·적 알림 0건 → 시작 10초 내 미지 색 = 동맹, 이후 = 적, 진영당 6색)
  7판 소급 분석 완료: 2승 5패, 오탐 의심 0/37(전 피격 알림 정황 일치), 팁 응답 58% ✅
- [ ] 자동 하이라이트 녹화 — 보류 (인코딩 부하 실측 게이트 후 결정)
