...
5. **주 경로** (MinimapReader): `lobby→inGame` 전이 후 첫 3게임초 내 미니맵에 나타나는 내 색이 아닌 색 클러스터(`pixels ≥ 3 && framesHeld ≥ 3`) = 동맹 — 팀 매치메이킹은 공유 시야가 기본이라 아군 기지가 시작부터 보이고, 적은 안개 속이라 보이지 않는다(0단계 5번 실측으로 확정). 각 클러스터의 최근접 스폰 = `allyBases`. 구현은 **미지 색 추론**으로 일반화(개별 색 다인전 실전 대응): 시작 10초 내 등장 미지 채도색 = 동맹 추정, 이후 = 적 추정, 표본 12px↑·기존 기준과 거리 90↑·진영당 6색. **게이트 3종(실전 확정 2026-08-25)**: ① **자원 색 제외** — 미니맵 미네랄 시안(실측 평균 RGB (53,221,247), 맨해튼 거리 90 미만)은 어느 진영으로도 추론 금지. 근거: 컴퓨터 1:1 투혼 2판에서 본진 미네랄이 시작 창의 '동맹'으로 추론 → `mode=.team` → "아군 피격" 76건 전건 오탐 ② **동맹 추론은 로비 경유 진입 + 로비 3인↑에서만** — 1:1(슬롯 2)엔 동맹이 없다. 빈 슬롯 폴백 없음(2026-08-27 확정 — 미파싱 명분으로 열어두면 1:1 유령 동맹 재발). 중반 진입(idle)은 10초 창의 전제(적은 안개 속) 자체가 깨지므로 동맹 추론 전면 금지 ③ **2프레임 지속 관측 후 채택** — 팔레트 전환·이펙트 1프레임 잔상 방어 ④ **적 채택은 "움직인 색"만** (2026-08-27 실전 확정: 내 색 Teal 판에서 가스 간헐천 초록이 '10초 후 미지 색 = 적' → "본진에 적" 0:14부터 16초 간격 53회, 실피격 0): 32×32 그리드 점유 이력에서 `everCells − maxConcurrent ≥ 3`이어야 채택 — 자원·중립 정지물은 색 대역 열거가 아니라 구조(적 군대는 반드시 움직인다)로 차단. 한계: 정지 방어선만 보이는 적 색은 병력 이동 시까지 지연. **부속 확정 2건**: 내 색 관측(dominantColor)은 미네랄 시안 픽셀 제외(내 색 오학습 차단), 동맹창 관측은 2인 게임에서 isAlly 강제 해제(체크 오독 → 컴퓨터가 '아군' → "1시 아군 피격" 실로그 6건 — 색 학습은 보존, `AllianceReader.sanitized`)
...
| ID | 우선순위 | 조건 | Refire |
|---|---|---|---|
| `supply.block` | warn | `(max-used)/rate < horizon` (게이트 통과값 기준, horizon = 반응 25s + 종족 건설시간 + 3s — P 47s/T·Z 53s) && **게임 5분 경과** (2026-08-25 사용자 확정 — 초반은 빌드가 손에 있어 침묵) | `cooldown(25)` |
| `minimap.flash` | urgent(내)·warn(아군) | FlashDetector 토글 클러스터 — 클러스터별 발화 (§6.2). **아군 피격은 warn 강등 + 전역 간격 12s, 내 피격 전역 간격 4s** (2026-08-25 실전 확정: 팀전 폭주 194발화/12분·큐 초과 131건 — 존별 쿨다운만으론 못 막음) | `cooldownPerPhrase(5)` — 아군 15 |
| `minimap.enemy` | warn | 내·아군 존 안 적(§6.4 판정) 클러스터 `pixels>=3 && framesHeld>=3` + **규칙 전역 간격 8s** (존 무관 — 위와 동일 폭주 억제) | `cooldownPerPhrase(10)` |
